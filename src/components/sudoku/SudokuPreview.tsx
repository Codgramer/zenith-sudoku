...
const GridLineX = ({top, width, makeBold}: {top: number; width: number; makeBold: boolean}) => (
  <div
    className={`absolute left-0 h-px transform -translate-y-1/2 ${makeBold ? "bg-primary-dark dark:bg-primary-light" : "bg-surface-secondary dark:bg-gray-600"}`}
    style={{
      width: `${width}%`,
      top: `${top}%`,
    }}
...
const GridLineY = ({left, height, makeBold}: {left: number; height: number; makeBold: boolean}) => (
  <div
    className={`absolute top-0 w-px transform -translate-x-1/2 ${makeBold ? "bg-primary-dark dark:bg-primary-light" : "bg-surface-secondary dark:bg-gray-600"}`}
    style={{
      height: `${height}%`,
      left: `${left}%`,
    }}
...
const SudokuPreviewGrid = ({width, height, hideLeftRight = false}: {width: number; height: number; hideLeftRight?: boolean}) => {
  return (
    <div>
      {SUDOKU_COORDINATES.map((i) => {
...
export const SudokuPreview = React.memo(
  ({
    sudoku,
    id,
    onClick,
    ...
    return (
      <div
        className="user-select-none hover:cursor-pointer group relative"
        tabIndex={id + 4}
        onClick={onClick}
        onKeyDown={onKeyDown}
        style={{
      ...
      >
        <div className="absolute z-10 top-0 left-0 w-full h-full flex items-center justify-center">
          <div style={{fontSize: size / 3}} className="font-bold text-text-primary dark:text-white opacity-80">
            {id}
          </div>
        </div>
