...
export function generateSudokuWithPrefilledCells(prefilledCells: number, randomFn: () => number): {
    sudoku: SimpleSudoku;
    iterations: number;
    error?: string;
} {
  if (prefilledCells < 17 || prefilledCells > 80) {
    return {
      sudoku: [],
      iterations: Infinity,
      error: "Number of prefilled cells must be between 17 and 80",
    };
  }

  let sudoku = createSolvableSudoku(randomFn);
  const solvedSudoku = cloneSudoku(sudoku);

  let coordinateList = generateCoordinateList(sudoku);
  while (coordinateList.length > prefilledCells) {
    const sampleXY = sample(coordinateList, randomFn);
    const [x, y] = sampleXY;
    coordinateList = coordinateList.filter(([cx, cy]) => cx !== x || cy !== y);
    ...
    if (isSudokuUnique(newSudoku)) {
      sudoku = newSudoku;
    }
  }

  const iterations = sudokuSolver(sudoku).iterations;
  return {
    sudoku,
    iterations,
  };
