...
const CLEAR_ALL_NOTES = "sudoku/CLEAR_ALL_NOTES"; // New action type
...
function sudokuReducer(state: SudokuState, action: SudokuAction): SudokuState {
  switch (action.type) {
    ...
    case CLEAR_ALL_NOTES:
      const newGridWithClearedNotes = state.current.map(cell => ({
        ...cell,
        notes: [],
      }));

      // Add to history
      const newHistoryClearedNotes = [newGridWithClearedNotes, ...state.history.slice(state.historyIndex)];
      return {
        current: newGridWithClearedNotes,
        history: newHistoryClearedNotes,
        historyIndex: 0,
      };
    case GET_HINT:
    case CLEAR_CELL:
    ...
    case CLEAR_NUMBER:
      ...
      // Add to history
      const newHistory = [newGrid, ...state.history.slice(0, state.historyIndex + 1)];
...
interface SudokuContextType {
  ...
  undo: () => void;
  redo: () => void;
  clearAllNotes: () => void; // New function added to context type
}
...
export function SudokuProvider({children, initialState = INITIAL_SUDOKU_STATE}: SudokuProviderProps) {
  ...
  const clearAllNotes = useCallback(() => { // New function implementation
